...
const moduleDetails = ({ moduleId }) => {
  ...
  const kanban = useSelector((state) => state.courses);
  const { comments, profiles, modules, selectedItem, userStory } =
    kanban;
  ...
  if (selectedItem !== false) {
    selectedData = modules.filter((item) => item.id === selectedItem)[0];
  ...
  const handleModalClose = (status) => {
    ...
    if (status) {
      handleDrawerOpen();
     // dispatch(deleteItem(selectedData.id, modules, columns, userStory,moduleId));
      dispatch(
        openSnackbar({
          open: true,
          message: "Module Deletedd",
