...
// assets
import { DownOutlined, EditTwoTone, GroupOutlined, RightOutlined, SendOutlined, UngroupOutlined, PlusOutlined } from '@ant-design/icons';
import AddButton from 'components/StyledButtons';
const tableTitle = "Employee";
// ==============================|| REACT TABLE ||============================== //

const EditableRow = ({ value: initialValue, row: { index }, column: { id, dataType }, editableRowIndex, groupNames}) => {
        ...
        <>
          {IsEditAble ? (
            ...
            >
              ...
              <MenuItem value={'LOA'}>
              <Chip color="success" label={'LOA'} size="small" variant="light" />
            </MenuItem>
...
function ReactTable({ columns, data, handleAddParticipant,groupNames,grabeditableRowIndex }) {

  const [controlledData, setControlledData] = useState(data); // Controlled data state

  // ... other existing code ...

  // Reset function to reset table state
  const resetTable = () => {
    setControlledData(data); // Reset the controlled data state
  };
  ...
  const initialState = useMemo(
    () => ({
      ...
      hiddenColumns: ['id', 'contact', 'country', 'fatherName', 'visits'],
  ...
  grabeditableRowIndex(editableRowIndex)
...
const EmployeesTable = ({ participants,index, project_parentGroup,groupNames }) => {
...
// ADD BUTTON DEPS
  ...
  function handleAddParticipant(newParticipant) {
    ...
    dispatch(addParticipant(participants,newParticipant,index));

  };

function grabeditableRowIndex(editableRowIndex){
    ...
    return (
      <>
        <DndProvider backend={isMobile ? TouchBackend : HTML5Backend}>
          ...
          >
            <ScrollX>
              <ReactTable columns={columns} data={participants} handleAddParticipant={handleAdd} groupNames={groupNames} grabeditableRowIndex={grabeditableRowIndex}/>
              <DragPreview />
            </ScrollX>
          </MainCard>
        </DndProvider>
        ...
        >
          <AddEmployee customer={customer} onCancel={handleAdd} handleAddParticipant={handleAddParticipant} project_parentGroup={project_parentGroup} />
        </Dialog>
      </>
    );
};

EmployeesTable.getLayout = function getLayout(page) {
