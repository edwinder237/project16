...
import { addParticipant, updateParticipant, removeParticipant } from 'store/reducers/projects';
...
// assets
import { DownOutlined, EditTwoTone, GroupOutlined, RightOutlined, SendOutlined, UngroupOutlined, PlusOutlined, MoreOutlined, DeleteOutlined } from '@ant-design/icons';

const tableTitle = "Participant";

// ==============================|| REACT TABLE ||============================== //

const EditableRow = ({ value: initialValue, row: { index }, column: { id, dataType }, editableRowIndex, groups }) => {
  ...
  const ShowGroup = (Value) => {
    if (typeof Value === 'string') {
      return (
        <Chip
          style={{ backgroundColor: Value.chipColor, color: '#fff' }}
          label={Value}
          size="small"
          variant="filled"
        />
      );
    } else if (Value !== null && Value.groupName !== undefined && Value.chipColor) {
      return (
        <Chip
          style={{ backgroundColor: Value.chipColor, color: '#fff' }}
          ...
          {IsEditAble ? (
            ...
            >
              <MenuItem value={value}>
                <Chip color="error" label="Individual" size="small" variant="filled" />
              </MenuItem>
              {groups.map((group) => (
...
const ColumnCell = ({ row, setEditableRowIndex, editableRowIndex, handleRemoveParticipant }) => {
    ...
    <>
      <Stack direction="row" alignItems="center" justifyContent="center" spacing={0}>
        ...
        <IconButton
          ...
          aria-controls={open ? `groups-action-button-${editableRowIndex}` : undefined}
          aria-haspopup="true"
...
function ReactTable({ columns, data, handleCRUD, groups }) {
  ...
  const defaultColumn = useMemo(() => ({ Filter: DefaultColumnFilter, Cell: (props) => <EditableRow {...props} groups={groups} /> }), []);
  const initialState = useMemo(
    () => ({
      filters: [{ id: 'status', value: '' }],
      hiddenColumns: ['id', 'email', 'note', 'progress', 'parentGroup', 'status'],
      columnOrder: ['selection', 'avatar', 'firstName', 'lastName', 'role', 'department', 'email', 'group'],
      pageIndex: 0,
      pageSize: 10
    }),
    []
    ...
    (hooks) => {
      hooks.allColumns.push((columns) => [
        ...columns,
        {
          ...
          Cell: (props) => <ColumnCell {...props} handleRemoveParticipant={handleRemoveParticipant} />
        }
      ]);
    }
  );
...
const ParticipantTable = ({ Project, index }) => {
  ...
  const handleCRUD = {
    ...
    handleRemoveParticipant: (uuid) => {
      //removes participant[index].uuid from Project.participants state
        ...
        const updatedParticipants = data.filter(participant => participant.uuid !== uuid);
  ...
  return (
    <>
      <DndProvider backend={isMobile ? TouchBackend : HTML5Backend}>
        ...
        >
          <ScrollX>
            <ReactTable columns={columns} data={data} handleCRUD={handleCRUD} groups={groups} />
            <DragPreview />
          </ScrollX>
        </MainCard>
      </DndProvider>
      ...
      >
        <AddParticipant customer={customer} groups={groups} onCancel={handleAdd} handleCRUD={handleCRUD} title={title} />
      </Dialog>
