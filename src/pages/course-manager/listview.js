...
function ReactTable({ columns, data, renderRowSubComponent, handleAdd }) {
  ...
  //const data = useMemo(() => {
    // Perform any expensive data transformations here
    //return items;
  //}, [items]);
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        Header: 'Contact',
        accessor: 'contact',
        Cell: "empty" //ContactCell if Phone Number is required
      },
      {
        Header: 'Duration',
        accessor: 'age',
  ...
  return (
    <Page title="Courses List">
      <MainCard content={false}>
        <ScrollX>
          {true && <ReactTable columns={columns} data={data} handleAdd={handleAdd} renderRowSubComponent={renderRowSubComponent} />}
        </ScrollX>
