...
function ReactTable({ columns, data, renderRowSubComponent, handleAdd }) {
  ...
  const {
  ...
  } = useTable(
    {
      ...
      data,
      filterTypes,
      initialState: { pageIndex: 0, pageSize: 5, hiddenColumns: ['avatar', ], sortBy: [sortBy] }
    },
    useGlobalFilter,
    useFilters,
    useSortBy,
...
const  CustomerListPage  = ()  => {
  ...
  //console.log('fetching output',data);
  ...
  return (
    <Page title="Courses List">
      <MainCard content={false}>
        <ScrollX>
          {true && <ReactTable columns={columns} data={data.length>0? data: [{name:"bob"},{name:"bob"}]} handleAdd={handleAdd} renderRowSubComponent={null} />}
        </ScrollX>
